...
class ShowSpider(scrapy.Spider):
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 0.5,
        'ITEM_PIPELINES': {'data_capture.pipelines.PaginatingPipeline': 300 },
    ...
    def start_requests(self):
        ...
        # Todo: Make show_ids.txt public, keep the ids updated
        show_ids = []
        with open('data/show_ids.txt') as f:
            for line in f:
                show_ids.append(line.strip())
        return (scrapy.Request('https://www.mydramalist.com/' + id)
                for id in show_ids)
    ...
    def parse(self, response: scrapy.http.Response):
        ...
        yield scrapy.Request(response.url + '/reviews',
                             callback=self.parse_reviews)
        yield scrapy.Request(response.url + '/recs',
    ...
    def parse_metadata(self, response: scrapy.http.Response):
        ...
        metadata['id'] = response.css('[data-type=title]::attr(data-id)')[
            0].extract()
        metadata['url'] = response.url
        metadata['title'] = field_selectors.css('[itemprop=name]::text')[
            0].extract()

        date_strings = field_selectors.css('li:nth-child(4)::text')[
            0].extract().split('-')
        ...
        metadata['duration'] = int(field_selectors.css('li:nth-child(7)::text')[
                                       0].extract().split()[0])

        metadata['average_score'] = int(10 * float(
                field_selectors.css('li:nth-child(1)::text')[
                    1].extract().strip()))
        metadata['num_users_rated'] = int(
                response.css('[itemprop=ratingCount]::attr(content)')[
                    0].extract())
        ...
        metadata['country'] = field_selectors.css('li:nth-child(2)::text')[
            0].extract().strip()
        metadata['num_episodes'] = int(
                field_selectors.css('li:nth-child(3)::text')[
                    0].extract().strip())
        metadata['rank'] = int(
                field_selectors.css('li:nth-child(2)::text')[1].extract()[2:])
        metadata['popularity'] = int(
                field_selectors.css('li:nth-child(3)::text')[1].extract()[2:])
    ...
    def parse_reviews(self, response: scrapy.http.Response):
        ...
        for selector in response.css('.review'):
            ...
            review['show_id'] = response.css(
                    '[property*=rid]::attr(content)').extract_first()
            review['show_title'] = response.css(
                    '[property*=title]::attr(content)').extract_first()
            review['date_posted'] = selector.css(
                    '.datetime::text').extract_first()

            review['num_votes'] = int(
                    selector.css(
                            '[class*=stats-helpful]::text').extract_first())

            review['overall_score'] = int(
                    float(selector.css('.score::text').extract_first()) * 10)
            ...
            review['review_text'] = ''.join(
                    selector.css('.review-body::text').extract())
            self.log('ok')
            yield review
    ...
    def parse_recommendations(self, response: scrapy.http.Response):
        ...
        for selector in response.css('[id*=rec_]'):
            ...
            show_ids = {response.css('[property*=rid]::attr(content)') \
                            .extract_first(),
